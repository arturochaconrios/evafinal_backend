...
class Camiseta {
    ...
    public static function precioFinal($camiseta_id, $cliente_id) {
        ...
        if (!$camiseta || !$cliente) {
            ...
            echo json_encode(["error" => "Camiseta o cliente no encontrado"]);
            return;
        }
        $precio_final = $camiseta['precio'];
        if ($cliente['categoria'] === 'Preferencial' && $camiseta['precio_oferta']) {
            $precio_final = $camiseta['precio_oferta'];
        } elseif ($cliente['porcentaje_oferta'] > 0) {
            $precio_final = $precio_final * (1 - $cliente['porcentaje_oferta'] / 100);
        }
